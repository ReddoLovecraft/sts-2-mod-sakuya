...
public class KnifeMagic: SakuyaCardModel
{
        protected override IEnumerable<IHoverTip> ExtraHoverTips => (new IHoverTip[1]
  {
	...
	public KnifeMagic() : base(1, CardType.Skill, CardRarity.Common, TargetType.AnyEnemy)
	{
	}
	protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
	{
        if(Owner.Creature.HasPower<KnifePower>())
		{
			...
			int cnt=Math.Min(kp.Amount,DynamicVars.Cards.IntValue);
			await kp.ThrowKnife(choiceContext,cardPlay.Target,KnifeType.AnyEnemy,1,cnt);
